...
class PowVideoResolver(ResolveUrl):
    ...
    pattern = r'(?://|\.)(powvideo\.(?:net|cc))/(?:embed-|iframe-|preview-)?([0-9a-zA-Z]+)'
    ...
    def get_media_url(self, host, media_id):
        ...
        if html:
            splice = re.search(r"\['splice'\]\(0x([0-9a-fA-F]*),0x([0-9a-fA-F]*)\);", html)
            sources = helpers.scrape_sources(html, patterns=[r'''src\s*:\s*["'](?P<url>http[^"']+\.mp4)["']'''])
