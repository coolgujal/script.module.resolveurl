...
class VidBomResolver(ResolveUrl):
    name = "vidbom"
    domains = ['vidbom.com', 'vidbm.com']
    pattern = r'(?://|\.)(vidbo?m\.com)/(?:embed[/-])?([A-Za-z0-9]+)'
    ...
    def get_media_url(self, host, media_id):
        ...
        if html:
            file_id = re.search(r"file_id',\s*'([^']+)", html)
            if file_id:
        ...
        if html:
            ...
            aa_text = re.search(r"""(ﾟωﾟﾉ\s*=\s*/｀ｍ´\s*）\s*ﾉ.+?;)\svar""", html, re.I)
            if aa_text:
                try:
    ...
    def get_url(self, host, media_id):
        return self._default_get_url(host, media_id, template='https://vidbm.com/{media_id}.html')
