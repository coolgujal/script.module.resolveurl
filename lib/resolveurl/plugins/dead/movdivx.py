...
class MovDivxResolver(ResolveUrl):
    ...
    pattern = r'(?://|\.)((?:movdivx|divxme|streamflv)\.com)/([0-9a-zA-Z]+)'
    ...
    def get_media_url(self, host, media_id):
        ...
        if html:
            ...
            if "method_free" not in data:
                data.update({'method_free': 'Submit Query'})
            _html = self.net.http_POST(web_url, headers=headers, form_data=data).content
            if _html:
                sources = helpers.scrape_sources(_html, patterns=[r'''file:\s*["'](?P<url>[^"']+)'''])
                if sources:
