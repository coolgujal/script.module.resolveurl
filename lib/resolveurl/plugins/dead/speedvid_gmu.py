...
def get_media_url(url, media_id):
    ...
    if html:
        ...
        aa_text = re.findall(r"""(ﾟωﾟﾉ\s*=\s*/｀ｍ´\s*）\s*ﾉ.+?)</SCRIPT>""", html, re.I)
        if aa_text:
            try:
                aa_decoded = ''
                for i in aa_text:
                    try:
                        aa_decoded += str(aadecode.decode(re.sub(r'\(+ﾟДﾟ\)+\s*\[ﾟoﾟ\]\)*\s*\+(.+?)\(+ﾟДﾟ\s*\)+\[ﾟoﾟ\]\)+', r'(ﾟДﾟ)[ﾟoﾟ]+\1(ﾟДﾟ)[ﾟoﾟ])', i)))
                    except:
                        pass
                href = re.search(r"""\.location\s*=\s*['"]\/([^"']+)""", aa_decoded)
                if href:
                    href = href.group(1)
                    if href.startswith("http"):
                        location = href
                    elif href.startswith("//"):
                        location = "http:%s" % href
                    else:
                        location = "http://www.speedvid.net/%s" % href
                    ...
                    sources = helpers.scrape_sources(_html, patterns=[r'''file\s*:\s*.["'](?P<url>(?=http://s(?:02|06))[^"'] + )'''])
                    if sources:
                        del headers['Cookie']
