...
class SyFyResolver(ResolveUrl):
    name = "SyFy"
    domains = ["syfy.com"]
    pattern = r"(?://|\.)(syfy\.com)/(\w+/videos/[a-zA-Z0-9_\-']+)"
    ...
    def get_media_url(self, host, media_id):
        ...
        if html:
            try:
                purl = re.compile(r'data-src="(.+?)"', re.DOTALL).search(html)
                purl = purl.group(1)
                purl = 'http:' + purl.replace('&amp;', '&')
                html = self.net.http_GET(purl, headers=headers).content
                purl = re.compile(r'<link rel="alternate" href=".+?<link rel="alternate" href="(.+?)"', re.DOTALL).search(html).group(1)
